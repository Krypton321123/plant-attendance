    ...
    return (
      <View style={styles.card}>
        <View style={[styles.avatar, isApproved ? styles.avatarApproved : styles.avatarPending]}>
          <Text style={[styles.avatarText, isApproved ? styles.avatarTextApproved : styles.avatarTextPending]}>{initials}</Text>
        </View>
        <View style={styles.cardInfo}>
          <Text style={styles.empName}>{item.EMPNAME} {item.EMPFNAME}</Text>
          <Text style={styles.empDesg}>{item.EMPDESG}</Text>
          <View style={styles.metaRow}>
            <View style={[styles.typePill, item.EMPTYPE === "SUPERVISOR" && styles.typePillSupervisor, item.EMPTYPE === "ADMIN" && styles.typePillAdmin]}>
              <Text style={[styles.typePillText, item.EMPTYPE === "SUPERVISOR" && styles.typePillTextSupervisor]}>{item.EMPTYPE}</Text>
            </View>
            {!item.DEVICEID && (
        ...
        <TouchableOpacity
          style={[styles.approveBtn, isApproved ? styles.approveBtnRevoke : styles.approveBtnApprove]}
          onPress={() => toggleApproval(item)} disabled={isToggling}
        >
          {isToggling
            ? <ActivityIndicator size="small" color={isApproved ? C.red : C.green} />
            : <Ionicons name={isApproved ? "close-circle-outline" : "checkmark-circle-outline"} size={22} color={isApproved ? C.red : C.green} />
          }
        </TouchableOpacity>
      </View>
    );
    ...
    <WithDrawer>
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        <View style={styles.header}>
          ...
          <TouchableOpacity style={styles.addBtn} onPress={() => router.push("/supervisor/create-employee")}>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchEmployees(); }} tintColor={C.primary} />}
            ListHeaderComponent={<Text style={styles.listHeader}>All Employees ({employees.length})</Text>}
...
const styles = StyleSheet.create({
  ...
  noDevicePill:         { backgroundColor: C.inputBg, borderRadius: 4, paddingHorizontal: 7, paddingVertical: 2 },
