...
export default function IndividualHomeScreen() {
  ...
  const confirmAbsent = async () => {
    ...
    try {
      ...
      fd.append("empId", employee.EMP_ID); fd.append("status", "A");
      const res  = await fetch(`${API_URL}/attendance/mark`, { method: "POST", headers: { Accept: "application/json" }, body: fd });
      const data = await res.json();
      if (res.ok) setTodayRecord({ STATUS: "A", CREATEDAT: new Date().toISOString(), LOCATION: null });
  ...
  const handleLogout = async () => {
    Alert.alert("Switch Account", "Are you sure?", [
      ...
      { text: "Switch", style: "destructive", onPress: async () => { await AsyncStorage.removeItem(STORAGE_KEYS.EMPLOYEE); router.replace("/auth/select-employee"); } },
    ]);
  };
  ...
  const markedTime = todayRecord?.CREATEDAT
    ? new Date(todayRecord.CREATEDAT).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })
    : null;
      ...
      <View style={styles.mainArea}>
        {!isMarked ? (
          <>
            ...
            <Text style={styles.mainSub}>Please mark your attendance for today before you leave.</Text>
            <TouchableOpacity style={styles.presentBtn} onPress={() => router.push({ pathname: "/camera/mark", params: { empId: employee.EMP_ID, empName: `${employee.EMPNAME} ${employee.EMPFNAME}`, isSelf: "true" } })}>
              <Ionicons name="camera-outline" size={22} color={C.textInverse} />
              <Text style={styles.presentBtnText}>Mark Present</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.absentBtn} onPress={() => Alert.alert("Mark Absent", "Are you sure you want to mark yourself absent?", [{ text: "Cancel", style: "cancel" }, { text: "Mark Absent", style: "destructive", onPress: confirmAbsent }])} disabled={markingAbsent}>
              {markingAbsent
                ? <ActivityIndicator color={C.red} size="small" />
                : <><Ionicons name="close-circle-outline" size={22} color={C.red} /><Text style={styles.absentBtnText}>Mark Absent</Text></>
              }
            </TouchableOpacity>
          </>
        ) : (
          <>
            <View style={[styles.markedIcon, isPresent ? styles.markedIconP : styles.markedIconA]}>
              <Ionicons name={isPresent ? "checkmark-circle" : "close-circle"} size={56} color={isPresent ? C.green : C.red} />
            </View>
            <Text style={styles.mainTitle}>{isPresent ? "You're Present Today" : "Marked Absent"}</Text>
            <Text style={styles.mainSub}>{isPresent ? "Your attendance has been recorded successfully." : "You have been marked absent for today."}</Text>
            {markedTime && (
...
const styles = StyleSheet.create({
  ...
  presentBtn: {
    ...
    backgroundColor: C.primary, borderRadius: 16, paddingVertical: 18, gap: 10, alignSelf: "stretch", marginBottom: 12,
  },
  presentBtnText: { color: C.textInverse, fontSize: 17, fontWeight: "800" },
  absentBtn: {
    flexDirection: "row", alignItems: "center", justifyContent: "center",
    backgroundColor: C.redBg, borderRadius: 16, paddingVertical: 18, gap: 10, alignSelf: "stretch",
    borderWidth: 1.5, borderColor: C.redLight,
  },
