...
type EmployeeRow = {
  ...
  EMPPROFILEPHOTO: string | null; todayStatus: "P" | "A" | null;
  markedAt: string | null; location: string | null;
};
...
export default function SupervisorHomeScreen() {
  ...
  useEffect(() => { loadSupervisor(); fetchAttendance(); }, []);
  ...
  const confirmAbsent = async () => {
    ...
    try {
      ...
      fd.append("status", "A");
      const res  = await fetch(`${API_URL}/attendance/mark`, { method: "POST", headers: { Accept: "application/json" }, body: fd });
      const data = await res.json();
      if (res.ok) {
        setEmployees(prev => prev.map(e => e.EMP_ID === pendingAbsent.EMP_ID ? { ...e, todayStatus: "A" } : e));
        setAbsentModal(false); setPendingAbsent(null);
    ...
    return (
      <View style={[styles.card, marked && styles.cardMarked]}>
        <View style={styles.cardLeft}>
          <View style={[styles.avatar, item.todayStatus === "P" && styles.avatarP, item.todayStatus === "A" && styles.avatarA]}>
            <Text style={[styles.avatarText, !!item.todayStatus && styles.avatarTextMarked]}>{initials}</Text>
          </View>
          <View style={{ flex: 1 }}>
            ...
            {item.todayStatus && (
              <View>
                <View style={[styles.statusBadge, item.todayStatus === "P" ? styles.badgeP : styles.badgeA]}>
                  <Text style={[styles.statusBadgeText, item.todayStatus === "P" ? styles.badgeTextP : styles.badgeTextA]}>
                    {item.todayStatus === "P" ? "✓ Present" : "✗ Absent"}
                  </Text>
                </View>
                {item.markedAt && (
                  <Text style={styles.markedAtText}>
                    {new Date(item.markedAt).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}
        ...
        {!marked && (
          <View style={styles.actions}>
            <TouchableOpacity style={styles.btnPresent} onPress={() => router.push({ pathname: "/camera/mark", params: { empId: item.EMP_ID, empName: `${item.EMPNAME} ${item.EMPFNAME}` } })}>
              <Text style={styles.btnPresentText}>P</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.btnAbsent} onPress={() => { setPendingAbsent(item); setAbsentModal(true); }}>
    ...
    <WithDrawer>
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        <View style={styles.header}>
          ...
          <TouchableOpacity style={styles.iconBtn} onPress={() => router.push("/supervisor/report")}>
            <Ionicons name="document-text-outline" size={20} color={C.primary} />
          </TouchableOpacity>
        </View>

        <View style={styles.statsRow}>
          <StatCard label="Present" value={present} color={C.green}  bg={C.greenBg}  border={C.greenLight} />
          <StatCard label="Absent"  value={absent}  color={C.red}    bg={C.redBg}    border={C.redLight}   />
          <StatCard label="Pending" value={unmarked} color={C.amber} bg={C.amberBg}  border={C.amberLight} />
        </View>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchAttendance(); }} tintColor={C.primary} />}
            ListHeaderComponent={<Text style={styles.listHeader}>Employees ({employees.length})</Text>}
          />
        )}
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalBox}>
              ...
              <Text style={styles.modalSub}>
                Mark <Text style={{ color: C.textPrimary, fontWeight: "700" }}>{pendingAbsent?.EMPNAME} {pendingAbsent?.EMPFNAME}</Text> as absent for today?
              </Text>
              <View style={styles.modalActions}>
                <TouchableOpacity style={styles.modalCancel} onPress={() => { setAbsentModal(false); setPendingAbsent(null); }}>
                  <Text style={styles.modalCancelText}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.modalConfirm} onPress={confirmAbsent} disabled={marking}>
                  {marking ? <ActivityIndicator color={C.textInverse} size="small" /> : <Text style={styles.modalConfirmText}>Mark Absent</Text>}
                </TouchableOpacity>
              </View>
            </View>
          </View>
...
const statStyles = StyleSheet.create({
  card:  { flex: 1, borderRadius: 14, padding: 14, alignItems: "center", borderWidth: 1 },
  value: { fontSize: 28, fontWeight: "800", marginBottom: 2 },
  label: { color: C.textSecondary, fontSize: 12, fontWeight: "600", letterSpacing: 0.5 },
});
...
const styles = StyleSheet.create({
  ...
  avatar:         { width: 44, height: 44, borderRadius: 22, backgroundColor: C.inputBg, justifyContent: "center", alignItems: "center" },
  ...
  statusBadge:    { alignSelf: "flex-start", borderRadius: 6, paddingHorizontal: 8, paddingVertical: 2, marginBottom: 2 },
  ...
  btnPresent: {
    width: 44, height: 44, borderRadius: 12, backgroundColor: C.greenBg,
    justifyContent: "center", alignItems: "center", borderWidth: 1, borderColor: C.greenLight,
  },
  btnPresentText: { color: C.green, fontSize: 15, fontWeight: "800" },
  btnAbsent: {
    width: 44, height: 44, borderRadius: 12, backgroundColor: C.redBg,
    justifyContent: "center", alignItems: "center", borderWidth: 1, borderColor: C.redLight,
  },
  btnAbsentText:  { color: C.red, fontSize: 15, fontWeight: "800" },
  modalOverlay:   { flex: 1, backgroundColor: "rgba(15,23,42,0.4)", justifyContent: "center", alignItems: "center", padding: 32 },
  modalBox: {
    backgroundColor: C.cardBg, borderRadius: 20, padding: 28,
    width: "100%", borderWidth: 1, borderColor: C.border, alignItems: "center",
    shadowColor: "#0F172A", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.12, shadowRadius: 24, elevation: 12,
  },
  modalIcon:        { width: 60, height: 60, borderRadius: 30, backgroundColor: C.redBg, justifyContent: "center", alignItems: "center", marginBottom: 16 },
  ...
  modalCancel: {
    flex: 1, backgroundColor: C.inputBg, borderRadius: 12, paddingVertical: 14,
