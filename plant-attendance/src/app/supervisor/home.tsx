...
export default function SupervisorHomeScreen() {
  ...
  const loadData = async (showRefreshing = false) => {
    ...
    try {
      ...
      if (data.success) {
        setCurrentShift(data.currentShift ?? "DAY");
  ...
  // ── Mark an EMPLOYEE absent (supervisor-initiated) ────────────────────────
  ...
  const confirmSelfAbsent = async () => {
    ...
    try {
      ...
      fd.append("status", "A");
      // No markedBy → treated as self-mark → supervisor's own approval is checked
  ...
  // Stats for the current shift across employees (not self)
  const present  = employees.filter(e =>  e.currentShiftMarked && (currentShift === "DAY" ? e.dayAttendance?.STATUS === "P" : e.nightAttendance?.STATUS === "P")).length;
  const absent   = employees.filter(e =>  e.currentShiftMarked && (currentShift === "DAY" ? e.dayAttendance?.STATUS === "A" : e.nightAttendance?.STATUS === "A")).length;
  ...
  const SelfAttendanceCard = () => {
    ...
    const markedTime = activeRecord?.CREATEDAT
      ? new Date(activeRecord.CREATEDAT).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })
      : null;
    ...
    return (
      <View style={selfStyles.card}>
        <View style={selfStyles.labelRow}>
          ...
          <View style={[selfStyles.shiftPill, currentShift === "NIGHT" && selfStyles.shiftPillNight]}>
            <Ionicons
              name={currentShift === "DAY" ? "sunny-outline" : "moon-outline"}
              size={10}
              color={currentShift === "DAY" ? C.amber : "#818CF8"}
            />
            <Text style={[selfStyles.shiftPillText, currentShift === "NIGHT" && selfStyles.shiftPillTextNight]}>
              {currentShift}
            </Text>
          </View>
        </View>
        ...
        {isCurrentShiftMarked ? (
          ...
          <View style={selfStyles.markedRow}>
            <View style={[selfStyles.statusDot, isPresent ? selfStyles.dotP : selfStyles.dotA]} />
            <Text style={[selfStyles.statusText, { color: isPresent ? C.green : C.red }]}>
            ...
            {activeRecord?.LOCATION && (
              <Text style={selfStyles.timeText} numberOfLines={1}>· {activeRecord.LOCATION}</Text>
            )}
          </View>
        ) : (
          /* ── Not yet marked ── */
                ...
                onPress={() =>
                  Alert.alert(
                    ...
                    [
                      ...
                      { text: "Mark Absent", style: "destructive", onPress: confirmSelfAbsent },
                    ]
                  )
                }
              >
        ...
        {currentShift === "NIGHT" && selfAttendance.dayRecord && (
          <View style={selfStyles.otherShiftRow}>
            ...
            <Text style={selfStyles.otherShiftText}>
              Day:{" "}
              <Text style={{ color: selfAttendance.dayRecord.STATUS === "P" ? C.green : C.red, fontWeight: "700" }}>
        ...
        {currentShift === "DAY" && selfAttendance.nightRecord && (
          <View style={[selfStyles.otherShiftRow, { borderColor: "#C7D2FE" }]}>
            <Ionicons name="moon-outline" size={11} color="#818CF8" />
            <Text style={selfStyles.otherShiftText}>
              Night:{" "}
              <Text style={{ color: selfAttendance.nightRecord.STATUS === "P" ? C.green : C.red, fontWeight: "700" }}>
                {selfAttendance.nightRecord.STATUS === "P" ? "Present" : "Absent"}
              </Text>
            </Text>
          </View>
        )}
      ...
      <View style={[styles.card, isMarked && styles.cardMarked]}>
        <View style={styles.cardLeft}>
          <View style={[
            ...
            <Text style={[styles.avatarText, isMarked && styles.avatarTextMarked]}>
              {initials}
            </Text>
          </View>
          ...
          <View style={{ flex: 1 }}>
            ...
            {isMarked && activeRecord && (
              <View>
                <View style={[styles.statusBadge, isPresent ? styles.badgeP : styles.badgeA]}>
                  <Text style={[styles.statusBadgeText, isPresent ? styles.badgeTextP : styles.badgeTextA]}>
                    {isPresent ? "✓ Present" : "✗ Absent"}
                  </Text>
                </View>
            ...
            {otherRecord && (
              <View style={[
                ...
                <Ionicons
                  name={currentShift === "NIGHT" ? "sunny-outline" : "moon-outline"}
                  size={10}
                  color={currentShift === "NIGHT" ? C.amber : "#818CF8"}
                />
                <Text style={[
                  styles.otherShiftPillText,
                  currentShift === "NIGHT" && styles.otherShiftPillTextNight,
                ]}>
                  {currentShift === "NIGHT" ? "Day" : "Night"}:{" "}
                  <Text style={{ fontWeight: "700", color: otherRecord.STATUS === "P" ? C.green : C.red }}>
        ...
        {!isMarked && (
          <View style={styles.actions}>
            ...
            <TouchableOpacity
              ...
              onPress={() => { setPendingAbsent(item); setAbsentModal(true); }}
            >
  ...
  return (
    <WithDrawer>
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        ...
        {/* Absent confirmation modal (for employees) */}
            ...
            <View style={styles.modalBox}>
              ...
              <View style={styles.modalActions}>
                <TouchableOpacity
                  ...
                  onPress={() => { setAbsentModal(false); setPendingAbsent(null); }}
                >
...
const selfStyles = StyleSheet.create({
  ...
  btnPresent: {
    ...
    backgroundColor: C.primary, borderRadius: 12, paddingVertical: 12, gap: 6,
  },
  ...
  otherShiftRow: {
    ...
    paddingTop: 10, borderTopWidth: 1, borderTopColor: C.amberLight,
  },
...
const styles = StyleSheet.create({
  ...
  modalBox: {
    backgroundColor: C.cardBg, borderRadius: 20, padding: 28, width: "100%",
    borderWidth: 1, borderColor: C.border, alignItems: "center",
    shadowColor: "#0F172A", shadowOffset: { width: 0, height: 8 },
    shadowOpacity: 0.12, shadowRadius: 24, elevation: 12,
  },
  modalIcon: {
    width: 60, height: 60, borderRadius: 30, backgroundColor: C.redBg,
