...
export default function MarkCameraScreen() {
  ...
  const { empId, empName, isSelf, markedBy } = useLocalSearchParams<{
    empId: string;
    empName: string;
    isSelf: string;
    markedBy: string; // supervisor's EMP_ID — present when supervisor marks someone else
  }>();
  ...
  // ── Shift label shown in the overlay ──────────────────────────────────────
  const getShiftLabel = (): "DAY" | "NIGHT" => {
    ...
    if (photoUri) {
      ...
      setTimeout(async () => {
        try {
          ...
          const uri = await captureRef(overlayRef, { format: "jpg", quality: 0.85 });
          setFinalUri(uri);
        } catch (e) {
          console.error(e);
  ...
  const handleUsePhoto = async () => {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        if (coords?.lat != null) formData.append("latValue",   String(coords.lat));
        if (coords?.lng != null) formData.append("longValue",  String(coords.lng));

        // ── Supervisor attribution ────────────────────────────────────────────
        // markedBy is set when a supervisor marks someone else's attendance.
        // Omitting it tells the server this is a self-mark.
        if (markedBy) formData.append("markedBy", markedBy);
        ...
        xhr.onload = () => {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            Alert.alert(
              "Success",
              `${empName}'s attendance marked as Present for the ${currentShift} shift!`,
  ...
  const PhotoOverlay = () => (
    ...
    >
      <View style={overlayStyles.row}>
        ...
        {/* Shift badge in the photo overlay */}
        <View style={[overlayStyles.shiftBadge, currentShift === "NIGHT" && overlayStyles.shiftBadgeNight]}>
          <Ionicons
            name={currentShift === "DAY" ? "sunny-outline" : "moon-outline"}
            size={10}
            color={currentShift === "DAY" ? "#F59E0B" : "#818CF8"}
          />
          <Text style={[overlayStyles.shiftText, currentShift === "NIGHT" && overlayStyles.shiftTextNight]}>
            {currentShift}
          </Text>
        </View>
      </View>
      ...
      {locationName ? (
        <View style={overlayStyles.row}>
          ...
          <Text style={overlayStyles.text} numberOfLines={2}>{locationName}</Text>
        </View>
      ) : null}
      {coords ? (
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.topBar}>
          <TouchableOpacity
            ...
            onPress={() => { setPhotoUri(null); setFinalUri(null); }}
          >
        ...
        <Text style={styles.subLabel}>
          ...
          {"  "}
          <Text style={{ color: currentShift === "DAY" ? C.amber : C.primary, fontWeight: "700" }}>
            [{currentShift}]
          </Text>
        </Text>

        <View style={styles.photoContainer} ref={overlayRef} collapsable={false}>
        ...
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity
            style={[styles.useBtn, (!finalUri || processing) && styles.useBtnDisabled]}
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.topBar}>
        ...
        <View style={styles.topBarCenter}>
          ...
          <View style={[styles.shiftChip, currentShift === "NIGHT" && styles.shiftChipNight]}>
            <Ionicons
              name={currentShift === "DAY" ? "sunny-outline" : "moon-outline"}
              size={12}
              color={currentShift === "DAY" ? C.amber : "#818CF8"}
            />
            <Text style={[styles.shiftChipText, currentShift === "NIGHT" && styles.shiftChipTextNight]}>
              {currentShift} SHIFT
            </Text>
          </View>
        </View>
...
const overlayStyles = StyleSheet.create({
  gradient: {
    ...
    paddingVertical: 14, paddingHorizontal: 16, gap: 5,
  },
  row:      { flexDirection: "row", alignItems: "center", gap: 6 },
  name:     { color: "#E8A020", fontSize: 13, fontWeight: "700", flex: 1 },
  text:     { color: "#FFF", fontSize: 12, flex: 1 },
  coords:   { color: "rgba(255,255,255,0.5)", fontSize: 10, marginTop: 2 },
  shiftBadge: {
    flexDirection: "row", alignItems: "center", gap: 3,
    backgroundColor: "rgba(245,158,11,0.25)", borderRadius: 6,
    paddingHorizontal: 6, paddingVertical: 2,
  },
  shiftBadgeNight: { backgroundColor: "rgba(129,140,248,0.25)" },
  shiftText:      { color: "#F59E0B", fontSize: 10, fontWeight: "800" },
  shiftTextNight: { color: "#818CF8" },
});
...
const styles = StyleSheet.create({
  ...
  topBarCenter: { alignItems: "center", gap: 4, flex: 1 },
  backBtn: {
    width: 36, height: 36, borderRadius: 10, backgroundColor: C.inputBg,
  ...
  shiftChip: {
    ...
    backgroundColor: C.amberBg, borderRadius: 8,
    paddingHorizontal: 8, paddingVertical: 3,
    borderWidth: 1, borderColor: C.amberLight,
  },
  shiftChipNight:     { backgroundColor: "#EEF2FF", borderColor: "#C7D2FE" },
  shiftChipText:      { color: C.amber, fontSize: 11, fontWeight: "800", letterSpacing: 0.5 },
  shiftChipTextNight: { color: "#818CF8" },
  subLabel: {
    color: C.textSecondary, fontSize: 14, textAlign: "center", marginVertical: 12,
  },
  empLabel: {
    flexDirection: "row", alignItems: "center", gap: 6, alignSelf: "center",
    backgroundColor: C.primaryLight, borderRadius: 20, paddingHorizontal: 14,
    paddingVertical: 6, marginBottom: 12, borderWidth: 1, borderColor: C.primaryMuted,
  },
  empLabelText:  { color: C.primary, fontSize: 13, fontWeight: "700" },
  camera:        { flex: 1 },
  photoContainer: {
    marginHorizontal: 20, borderRadius: 20, overflow: "hidden", height: 400,
    backgroundColor: C.border, position: "relative",
  ...
  shutterArea: {
    paddingBottom: 40, paddingTop: 20, alignItems: "center", gap: 16,
    backgroundColor: C.cardBg, borderTopWidth: 1, borderTopColor: C.border,
  },
  locationChip: {
    flexDirection: "row", alignItems: "center", gap: 5,
    backgroundColor: C.inputBg, borderRadius: 20, paddingHorizontal: 12,
    paddingVertical: 6, maxWidth: 280, borderWidth: 1, borderColor: C.border,
  },
  locationChipText: { color: C.textSecondary, fontSize: 12, flex: 1 },
  shutterBtn: {
    width: 72, height: 72, borderRadius: 36,
    borderWidth: 4, borderColor: C.primary, justifyContent: "center", alignItems: "center",
  },
  shutterInner: { width: 54, height: 54, borderRadius: 27, backgroundColor: C.primary },
  actionRow:    { flexDirection: "row", marginHorizontal: 20, marginTop: 20, gap: 12 },
  retakeBtn: {
    flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center",
    backgroundColor: C.cardBg, borderRadius: 14, paddingVertical: 16, gap: 8,
    borderWidth: 1, borderColor: C.border,
  },
  retakeBtnText: { color: C.textSecondary, fontSize: 15, fontWeight: "600" },
  useBtn: {
    flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center",
    backgroundColor: C.green, borderRadius: 14, paddingVertical: 16, gap: 8,
  },
  useBtnDisabled: { backgroundColor: C.border },
  useBtnText:     { color: C.textInverse, fontSize: 15, fontWeight: "800" },
  permBox: { flex: 1, justifyContent: "center", alignItems: "center", gap: 16, padding: 40 },
  permIconWrap: {
    width: 80, height: 80, borderRadius: 24, backgroundColor: C.primaryLight,
    borderWidth: 1, borderColor: C.primaryMuted, justifyContent: "center", alignItems: "center",
  },
  ...
  permBtn:   { backgroundColor: C.primary, borderRadius: 12, paddingVertical: 14, paddingHorizontal: 28 },
